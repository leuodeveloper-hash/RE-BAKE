 ...
 *
 * 사용 방법:
 * node scripts/downloadFigmaImages.mjs [Figma URL with node-id]
 */
...
function extractFileKeyFromUrl(figmaUrl) {
...
async function getFigmaNodes(fileKey, nodeIds) {
  if (!FIGMA_TOKEN) {
    throw new Error(
      'FIGMA_PERSONAL_ACCESS_TOKEN이 설정되지 않았습니다. .env 파일을 확인하세요.',
    );
  }
...
async function getFigmaImages(fileKey, nodeIds, scale = 2) {
  if (!FIGMA_TOKEN) {
    throw new Error(
      'FIGMA_PERSONAL_ACCESS_TOKEN이 설정되지 않았습니다. .env 파일을 확인하세요.',
    );
...
async function main() {
  ...
  if (args.length === 0) {
    console.log(
      '사용 방법: node scripts/downloadFigmaImages.mjs [Figma URL with node-id]',
    );
    console.log('');
    console.log('예시:');
    console.log(
      '  node scripts/downloadFigmaImages.mjs "https://www.figma.com/design/ce9cxzvQSvJoPldTSZIiGG/RE-BAKE?node-id=73553-102328"',
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (!nodeId) {
      console.error(
        'URL에 node-id가 없습니다. ?node-id=xxx 형식으로 제공하세요.',
      );
      process.exit(1);
    }
    ...
    console.log(
      `\n✅ 완료! ${downloadCount}개 이미지가 assets/images/recipes/ 폴더에 저장되었습니다.`,
    );
  } catch (error) {
